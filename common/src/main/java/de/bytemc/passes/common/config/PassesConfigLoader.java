...
public class PassesConfigLoader {
    ...
    private static final Gson GSON = new GsonBuilder()
        .setPrettyPrinting()
        .serializeNulls()
        .registerTypeAdapter(Pass.class, new GsonPass())
    ...
    private static final PassesConfig DEFAULT_CONFIG = new PassesConfig(
        Sets.newHashSet(
            new DefaultPass(0, Icon.builder("STONE").build(), DEFAULT_LEVELS, DEFAULT_LEVEL_CONFIGURATION),
            new EventPassImpl(1, Icon.builder("STONE").build(), new Date(0L), new Date(), DEFAULT_LEVELS, DEFAULT_LEVEL_CONFIGURATION)
        ),
